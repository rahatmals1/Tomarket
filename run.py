                    ...
                    async with session.post(url=url, headers=headers, data=data, ssl=False) as response:
                        ...
                        if 'status' in play_game:
                            if play_game['status'] == 0:
                                self.print_timestamp(
                                    f"{Fore.BLUE + Style.BRIGHT}[ Game Started ][Join Telegram:- https://t.me/InstantEarning169]{Style.RESET_ALL}"
                                    f"{Fore.WHITE + Style.BRIGHT} | {Style.RESET_ALL}"
...
if __name__ == '__main__':
    try:
        ...
        tomarket.print_timestamp(f"{Fore.YELLOW + Style.BRIGHT}[ Select an option ] | {Style.RESET_ALL}"
       ) 
        tomarket.print_timestamp(
      f"{Fore.GREEN + Style.BRIGHT}[ Script By RahatMals]{Style.RESET_ALL}")
        tomarket.print_timestamp(
